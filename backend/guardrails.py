...
class GuardrailSystem:
    ...
    async def check_input_guardrail(self, question: str) -> GuardrailResult:
        ...
        if self.api_key and settings.guardrail_enabled:
            try:
                prompt = f"""You are a content filter for an educational math tutoring system.
                
Analyze this question and determine if it is:
1. Related to mathematics, physics, chemistry, or science education
2. Appropriate for students (no harmful, offensive, or inappropriate content)
3. A genuine question seeking educational help
